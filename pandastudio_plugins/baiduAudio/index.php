...
function add_baiduAudio_translation_array($translation_array)
{
    ...
    if ($bd_response === false) {
        ...
        $response = wp_remote_get($bd_url);
        if (!is_wp_error($response)) {
            $body = wp_remote_retrieve_body($response);
            $bd_response = json_decode($body, true);
            if (!empty($bd_response['access_token'])) {
                set_cache('bd_audio_tok', [
                    'access_token' => $bd_response['access_token'],
                    'session_key'  => isset($bd_response['session_key']) ? $bd_response['session_key'] : ''
                ], $bd_response['expires_in'] * 0.9);
                $bd_response = get_cache('bd_audio_tok');
            }
        }
    }
    $cached_tok = is_array($bd_response) ? $bd_response : array('access_token' => '', 'session_key' => '');
    $translation_array['baiduAudio'] = array_merge($cached_tok, array(
        'spd' => get_option('baidu_ai_audio_spd', 5),
        'pit' => get_option('baidu_ai_audio_pit', 5),
        'per' => get_option('baidu_ai_audio_per', 0),
        'enable' => get_option('baidu_ai_audio_enable') == 'checked'
    ));
